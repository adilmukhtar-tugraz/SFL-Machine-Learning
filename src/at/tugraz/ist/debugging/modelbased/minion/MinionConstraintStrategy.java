...
//				System.out.println("TimeSpan: "+timeSpan.toString());
	...
	public ConstraintStrategyResult solveConstraints(
		...
		int i =0;
		for (i = 1; i <= minionInfo.getNumberAbnormalVariables(); i++) {
			if (ConstraintStrategyConfiguration.useEarlyTermination() && solutions.size() > 0)
				break;
			
			if(ConstraintStrategyConfiguration.getMaxDiagnosesSize()!=-1 && i>ConstraintStrategyConfiguration.getMaxDiagnosesSize())
				break;

//			if (i > 1)
//				break;
			
			System.out.println("compute diagnoses of size "+i);

			MinionCaller minionCaller = MinionCaller.create(COMMUNICATION_MODE);
			minionInfo.setSolutionSize(i);
...
//			}
			try {
				result.addSolvingTime(i, getComputationTime(result2.getSolution()));
        ...
        result.setMaxDiagnosisSize(i-1);
		return result;
	}
